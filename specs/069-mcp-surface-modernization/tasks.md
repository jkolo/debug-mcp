...
- [X] T089 Ran every quickstart.md scenario live over stdio with a purpose-built Python JSON-RPC harness (`scratchpad/t089/{harness,scenarios}.py`, not committed — reproducible from this note), driving a freshly-built `dotnet run --project DebugMcp --no-build` process. 38/38 assertions passed. Two premise corrections found and worked around rather than silently reproduced:
  ...
  - Scenario results: **S1** (progress+cancel) — first `notifications/progress` at 0.01s (via `debug_launch`) and 0.03s (via a live `resharper_inspect_solution` run against `tests/ReSharperSampleApp/ReSharperSampleApp.sln`, cached engine 2026.2.1, no download); zero notifications when no token supplied; **request-level cancellation exercised for real** — sent `notifications/cancelled {requestId, reason}` ~3s into a live `resharper_inspect_solution` run (progress already observed), the cancelled request never answered (per spec — not asserted on), no orphaned `jb` process left behind (`ps aux` checked), and the next call (`tools/list`) succeeded in <5s, proving the linked-CTS plumbing built in US5 actually gets exercised by the SDK's own cancellation notification, not just by a same-tool timeout. **S2** (deferred tasks) — `resultType: "task"` returned, task reached `completed`, byte-identical normalized result between deferred and direct paths (FR-014), cancel acknowledged and reached a terminal status, fabricated task id returned a distinguishable error, non-qualifying tool (`timeline_query`) ignored the declared extension. The expired-vs-not-found half of FR-012 (step 6) was **not** re-exercised live — `ttlMs` is 3600000 (1h), making a live expiry impractical — but it has direct Unit-tier coverage: `tests/DebugMcp.Tests/Unit/Tasks/McpTasksHarnessTests.cs::ExpiredTaskId_ThrowsADifferentErrorThanAnUnknownTaskId`, part of the 1620 green in T090. **S3** (result contracts) — 39/39 tools carry `outputSchema`; `code_load` and a deliberate `variables_get` failure both validated `structuredContent`/`content[0]`/`isError`+`success:false`+documented `code`; `content[0].text` parses standalone with the same fields (backward-compat proof). Step 2's "call each tool, validate structuredContent" was exercised live for the tools these scenarios actually call (`code_load`, `variables_get`, `debug_launch`/`debug_disconnect`, `resharper_inspect_solution`, `timeline_query`, `tasks/get`); the remaining tools' schema conformance rests on the Contract tier (`LegacyTextContractTests`, `AsyncStackTraceContractTests`), which is what actually asserts every tool's structured output against its own schema. **S4** (enrichment) — `NullDereference` fixture launched live, exception breakpoint set, `debugger/breakpointHit` observed, `exception_get_context` and `stacktrace_get` both carried `ranking` with the fault frame (index 0) ranked first, matching `expected-answers.json`. **S5** (timeouts) — every blocking tool's timeout param present/absent per `TimeoutPolicy.Specs` confirmed over the wire via `tools/list`; a 1ms `debug_launch` timeout produced a `TIMEOUT` error naming the budget within 5s and left the session usable for an immediate next launch; `debug_launch`'s schema documents its 30000ms default and `resharper_inspect_solution`'s documents its own 300s default (not 30000) — both read from the live `tools/list` response, not source.
  ...
  - **quickstart.md itself corrected** for the two premise gaps found above (Scenario 2's handshake-free requirement, the stale `TaskSupport = Forbidden` wording) — see quickstart.md's own diff, so the next person following it doesn't hit the same `-32600` wall this session spent several tool calls diagnosing.
