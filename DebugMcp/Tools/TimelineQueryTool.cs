...
public sealed class TimelineQueryTool
{
    ...
    public Task<string> TimelineQueryAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(eventTypes))
            {
                ...
                {
                    return Task.FromResult(JsonSerializer.Serialize(new { success = false, error = "Invalid eventTypes — expected JSON array of strings, e.g. [\"breakpoint_hit\"]" }));
                }
            }
